...
(function($, Drupal) {
  ...
  if ($('.JawardsSlide .splide').length > 0) {
    var splide = new Splide( '.splide', {
        type: $('.sliderAwards .splide__list .splide__slide').length > 2 ? 'loop' : 'slide',
  ...
  Drupal.behaviors.petronasParagraphMilestones = {
    attach: function(context, settings) {
        $('.JawardsSlideBodyList a, .JGeneralBodyList a, .awardsTabListItem a').once('openOverlay').on('click', function(e){
            e.preventDefault();
            $('body').addClass('noScroll');
            if($(this).attr('href').indexOf('#') >= 0){
                var final = $(this).attr('href').replace('#', '');
        ...
        $('.JgeneralPopupTD').once('clickPopupTD').on('click', function(event) {
          event.stopPropagation();
        });
    },
    stopMedias: function() {
  ...
  Drupal.behaviors.petronasViewsAwardsMap = {
    attach: function(context, settings) {
        ...
        var syncFilters = function() {
            ...
            var exposedForms = $('.view-milestones-v2 .views-exposed-form', context);
        ...
        $('.awardsTabMapFilter select').once('awardsTabMapFilter').on('change', function() {     
            var selectedValue = $(this).val(); 
            $('.awardsTabMaps .awardsTabMapsInner').hide();
            $('.awardsTabMaps .awardsTabMapsInner[data-tabs="' + selectedValue + '"]').show();

            $('.view-milestones-v2 .views-exposed-form select[name="country"]').each(function() {
                $(this)
                    .val(selectedValue)
                    .trigger('change');
            });
