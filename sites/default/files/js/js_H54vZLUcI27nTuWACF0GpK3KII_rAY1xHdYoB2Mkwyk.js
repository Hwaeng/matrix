...
(function ($, Drupal) {
  ...
  if ($(".JawardsSlide .splide").length > 0) {
    var splide = new Splide(".splide", {
      type:
        $(".sliderAwards .splide__list .splide__slide").length > 2
          ? "loop"
          : "slide",
      rewind: $(".sliderAwards .splide__list .splide__slide").length <= 2,
      perPage: 5,
      perMove: 1,
      focus: "center",
      // dragMinThreshold: 50
    ...
    /*splide.on( 'active', function () {
        ...
        // console.log(currentSlide, $('.JawardsSlide .is-active.is-visible .JawardsTabs').data("slick-index"));
        var current = $('.JawardsSlide .is-active.is-visible .JawardsTabs').data("page");
        console.log('current1:', current);
        $('.JawardsSlideBody').fadeOut(300);
        console.log($('.' + current));
        setTimeout(function() {
           $('div[data-page="' + current + '"]').fadeIn(300)
        }, 300);
    ...
    splide.on("moved", function () {
      setTimeout(function () {
        // console.log(currentSlide, $('.JawardsSlide .is-active.is-visible .JawardsTabs').data("slick-index"));
        var current = $(
          ".JawardsSlide .is-active.is-visible .JawardsTabs",
        ).data("page");
        // $('.JawardsSlideBody').fadeOut(300);
        $(".JawardsSlideBody").hide();
  ...
  Drupal.behaviors.petronasParagraphMilestones = {
    attach: function (context, settings) {
      $(".JawardsSlideBodyList a, .JGeneralBodyList a, .awardsTabListItem a")
        .once("openOverlay")
        .on("click", function (e) {
          e.preventDefault();
          $("body").addClass("noScroll");
          if ($(this).attr("href").indexOf("#") >= 0) {
            ...
            // if($('.JgeneralPopupContainer').attr('id') == final){
            //     $(this)
            // }
          }
        });
      ...
      $(".JgeneralPopupClose")
        ...
        .on("click", function () {
          $("body").removeClass("noScroll");
          $(this).parent().parent().parent().parent().fadeOut();
          Drupal.behaviors.petronasParagraphMilestones.stopMedias();
        });

      $(document)
        .once("docCloseOverlay")
        .on("click", function (event) {
          if ($(event.target).closest(".JgeneralPopupContainer").length > 0) {
            $("body").removeClass("noScroll");
...
(function ($, Drupal) {
  ...
  Drupal.behaviors.petronasViewsAwardsMap = {
    attach: function (context, settings) {
      /** Start sync filter implementation **/
      var syncFilters = function () {
        // Loop through each form and bind a change event to the filter inputs
        var exposedForms = $(
          ".view-milestones-v2 .views-exposed-form",
          context,
        );
        exposedForms.each(function () {
          var currentForm = $(this);

          // Bind change event to all select elements in the current form
          currentForm.find("select").on("change", function () {
            var filterName = $(this).attr("name");
            var filterValue = $(this).val();

            // Loop through all other exposed forms to sync the filter
            exposedForms.not(currentForm).each(function () {
              var otherForm = $(this);
              var otherFilter = otherForm.find('[name="' + filterName + '"]');

              if (otherFilter.length && otherFilter.val() !== filterValue) {
                otherFilter.val(filterValue);

                // Use setTimeout to ensure AJAX trigger happens after syncing the value
                setTimeout(function () {
                  otherFilter.trigger("change");
                }, 100); // Small delay to allow proper syncing
              }
            });
          });
        });
      ...
      $(".awardsTabMapFilter select")
        .once("awardsTabMapFilter")
        .on("change", function () {
          var selectedValue = $(this).val();
          $(".awardsTabMaps .awardsTabMapsInner").hide();
          $(
            '.awardsTabMaps .awardsTabMapsInner[data-tabs="' +
              selectedValue +
              '"]',
          ).show();

          $(
            '.view-milestones-v2 .views-exposed-form select[name="country"]',
          ).each(function () {
      ...
      $(document).ajaxComplete(function () {
        syncFilters(); // Reinitialize the event listeners after AJAX updates the DOM
      });

      /** End sync filter implementation **/

      /*$('.awardsTabListInner.all .awardsTabListItem').on('click', function() {
            var target = $(this).attr('data-target');  // Get the data-target attribute

            $('.awardsTabMapFilter select').val(target).trigger('change');

            if (target) {
                $('.awardsTabMaps .awardsTabMapsInner').hide();
                $('.awardsTabMaps .awardsTabMapsInner[data-tabs="' + target + '"]').show();
            }
        });*/

      $(".awardsTabMapsInner.all .map-points-item").on("click", function () {
        var target = $(this).attr("data-target"); // Get the data-target attribute
        $(".awardsTabMapFilter select").val(target).trigger("change");
        ...
        if (target) {
          $(".awardsTabMaps .awardsTabMapsInner").hide();
          $(
            '.awardsTabMaps .awardsTabMapsInner[data-tabs="' + target + '"]',
          ).show();
        }
      });
    },
