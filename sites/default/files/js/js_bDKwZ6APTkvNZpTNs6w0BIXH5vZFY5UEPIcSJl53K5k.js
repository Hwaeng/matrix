    ...
    attach: function(context, settings) {
      if ($("#search-form, #search-form-modal").length > 0) {
        ...
        if (!initSearchResult) {
          ...
          const searchClient = algoliasearch('IK2A979H8U', 'b744df046e750c5d48467cebf1fa0adc');
    ...
    attach: function (context, settings) {
      $('.paragraph--type--inner-portfolio-links', context)
        ...
        .each(function (index) {
          ...
          $(this).find('.view-all-links').once('openOverlay').on('click', function(e){
              e.preventDefault();
              $('body').addClass('noScroll');
              if($(this).attr('href').indexOf('#') >= 0){
                  var final = $(this).attr('href').replace('#', '');
          ...
          $('.JgeneralPopupClose').once('closeOverlay').on('click', function(){
              $('body').removeClass('noScroll');
              $(this).parent().parent().parent().parent().fadeOut();
          })
        
          $(document).once('docCloseOverlay').on("click", function(event){
...
(function($, Drupal) {
  ...
  Drupal.behaviors.petronasParagraphSpotlights = {
    attach: function(context, settings) {
        ...
        $('.paragraph--type--inner-spotlights').once('initSpotlightPara').each(function() {
          let $spotlightPara = $(this);
          $spotlightPara.find('.accordion-button').on('click', function(e) {
            $spotlightPara.find('.image').css('background-image', 'url("' + $(e.target).attr('data-image') + '")');

            $spotlightPara.find('.accordion-item').each(function() {
              $(this).find('.accordion-image').removeClass('d-block').addClass('d-none');
            });

            if (!$(e.target).hasClass('collapsed')) {
              $(e.target).closest('.accordion-item').find('.accordion-image').removeClass('d-none').addClass('d-block');
            }
          });
        });
...
(function($, Drupal) {
  ...
  Drupal.behaviors.petronasParagraphMultiColContentV2 = {
    attach: function(context, settings) {
      ...
      $('.column-item-content', context).once('initMulColContentHeight').each(function () {
        ...
        function adjust() {
          if (mediaQuery.matches) {
            ...
            if (containerHeight > 0) {
              const imagePercentage = ((containerHeight - contentHeight) / containerHeight) * 100;
...
(function($, Drupal) {
  ...
  Drupal.behaviors.petronasParagraphMediaFrontpage = {
    attach: function(context, settings) {
        ...
        $('.paragraph--type--media-frontpage', context).once('innerMediaFrontpageOnLoad').each(function () {
            const $para = $(this);
            const $listing = $('.paragraph--type--media-frontpage .owl-carousel');
            const $itemCount = $('.paragraph--type--media-frontpage [data-featured-wide="1"]').length > 0 ? 1 : 2;
            $listing.owlCarousel({
                items: 1,
                dots: false,
                margin: 24,
                ...
                onInitialized: function(event) {
                  ...
                  if (!$carousel.find('.owl-controls-wrapper').length) {
                    const $wrapper = $('<div class="owl-controls-wrapper position-relative mt-3"></div>');
                    $nav.add($dots).wrapAll($wrapper);

                    const $controlsWrapper = $carousel.find('.owl-controls-wrapper');
                    $controlsWrapper.append('<div class="carousel-footer-links news-link-wrapper d-none d-md-block">\
                <a href="/media/media-releases" class="news-link link-read-more smaller animate-icon">\
                    <i></i>\
                    <p>' + Drupal.t('View all') + '</p>\
                </a>\
            </div>');
                  }
                }
      ...
      $('.paragraph--type--inner-banner-carousel', context)
        ...
        .each(function () {
          ...
          $bannerImageCarousel.on('mouseover', function () {
            $navItems.find('.progress-bar, .clip-progress').css('transition-play-state', 'paused');
          });
          $bannerImageCarousel.on('mouseleave', function () {
            $navItems.find('.progress-bar, .clip-progress').css('transition-play-state', 'running');
          });
        });
    },
  };
